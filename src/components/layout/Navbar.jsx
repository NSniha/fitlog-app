...
export default function Navbar() {
    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-[#20242b] bg-[#0b0d10]/95 backdrop-blur-xl">
            <Container>
                <nav className="relative flex h-18 items-center justify-between md:h-20">
                    <Link href="/" onClick={() => setMenuOpen(false)} aria-label="FitLog home" className="flex shrink-0 items-center">
                    ...
                    <div className="hidden items-center gap-7 md:flex">
                        <Link href="/my-plan" onClick={() => handleNavClick("plan")} className="flex items-center gap-2.5 text-white! transition-colors duration-300 ease-out hover:text-[#c8ff00]!">
            ...
            <div id="mobile-navigation" className={`absolute left-0 top-full w-full overflow-hidden border-b border-[#20242b] bg-[#0b0d10] shadow-[0_20px_40px_rgba(0,0,0,0.35)] transition-all duration-300 ease-out md:hidden ${menuOpen ? "visible max-h-85 opacity-100" : "invisible max-h-0 opacity-0"}`}>
                <Container className="py-4">
                    <div className="rounded-2xl bg-[#14171c] p-2">
                        <Link href="/my-plan" onClick={() => handleNavClick("workouts")} className={`flex min-h-13 items-center rounded-xl px-4 text-[15px] font-medium transition-colors duration-300 ease-out hover:text-[#c8ff00]! ${activeNav === "workouts" ? "text-[#c8ff00]!" : "text-white!"}`}>
                            Workouts
                        </Link>

                        <Link href="/my-plan" onClick={() => handleNavClick("plan")} className={`flex min-h-13 items-center rounded-xl px-4 text-[15px] font-medium transition-colors duration-300 ease-out hover:text-[#c8ff00]! ${activeNav === "plan" ? "text-[#c8ff00]!" : "text-white!"}`}>
                            My Plan
                        </Link>
                        ...
                        <div className="mt-2 border-t border-[#262a31] px-2 pt-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <Link href="/my-plan" onClick={() => handleNavClick("plan")} className="flex h-12.5 items-center justify-between rounded-xl bg-[#0e1115] px-4 text-white! transition-colors duration-300 hover:text-[#c8ff00]!">
                                    <span className="text-[13px] font-medium">Plan</span>
